...
export class AuthService {
  ...
  public isSessionCreated(): boolean {
    return !!this.localStorageService.getItem<string>(this.EXPIRATION_KEY);
  }
  ...
  public isSessionExpired(): boolean {
    const expiration = this.localStorageService.getItem<string>(this.EXPIRATION_KEY);
    ...
    if (Date.now() > parseInt(expiration, 10)) {
      this.localStorageService.removeItem(this.EXPIRATION_KEY);
  ...
  private hydrateSession(): void {
    if (this.isSessionExpired()) {
      ...
      const storedSession = this.localStorageService.getItem<User>(this.SESSION_KEY);
      if (storedSession) {
  ...
  public login(email: string, password: string): boolean {
    ...
    if (userMatch) {
      ...
      this.localStorageService.setItem(this.SESSION_KEY, JSON.stringify(userMatch.profile));
      this.localStorageService.setItem(this.EXPIRATION_KEY, (Date.now() + this.TOKEN_LIFESPAN_MS).toString());
